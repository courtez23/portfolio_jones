...
function ParallaxNode({
  ...
  const opacity = useTransform(scrollProgress, [0, 0.7, 1], [node.opacity, node.opacity * 0.6, 0])
...
export default function ParticleNetwork({ scrollProgress }: ParticleNetworkProps) {
  ...
  const nodeElements = useMemo(
    () =>
      NODES.map((node) => (
        <ParallaxNode key={node.id} node={node} scrollProgress={scrollProgress} />
      )),
    [scrollProgress]
  )
